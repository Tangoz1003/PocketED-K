...
if __name__ == "__main__":
    # 模型测试代码 - 这些参数应该从配置文件获取
    # 这里仅用于测试模型结构的完整性
    model = Net1D(
        in_channels=12,  # 输入通道数（12导联ECG）
        base_filters=64,  # 基础滤波器数量，可选：32, 64
        ratio=1,  # 通道比例
        filter_list=[64,160,160,400,400,1024,1024],  # 各阶段滤波器数量列表
        # 备选配置：[16,32,32,80,80,256,256], [32,64,64,160,160,512,512]
        m_blocks_list=[2,2,2,3,3,4,4],  # 各阶段块数量列表
        # 备选配置：[2,2,2,2,2,2,2]
        kernel_size=16,  # 卷积核大小
        stride=2,  # 步长
        groups_width=16,  # 分组宽度
        verbose=True,  # 是否输出详细信息
        use_bn=False,  # 是否使用批归一化
        use_do=False,  # 是否使用Dropout
        n_classes=150  # 分类数量
    )
    print("模型结构:")
    print(model)
